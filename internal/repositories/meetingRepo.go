...
func NewMeetingRepo(db *bun.DB) MeetingRepo {
...
func (mr *meetingRepository) GetMeetingByID(ctx context.Context, meetingID int) (*models.Meeting, error) {
	...
	if err != nil {
		...
		log.Error().Err(err).Int("meeting_id", meetingID).Msg("Failed to get meeting by ID")
		return nil, err
	}
	return meeting, nil
}
...
func (mr *meetingRepository) UpdateMeeting(ctx context.Context, meeting *models.Meeting) error {
	...
	if err != nil {
		log.Error().Err(err).Int("meeting_id", meeting.ID).Msg("Failed to update meeting")
		return err
	}
	return nil
}
...
func (mr *meetingRepository) DeleteMeeting(ctx context.Context, meetingID int) error {
	...
	if err != nil {
		log.Error().Err(err).Int("meeting_id", meetingID).Msg("Failed to delete meeting")
		return err
	}
	return nil
}
...
func (mr *meetingRepository) AddParticipantToMeeting(ctx context.Context, meetingID, userID int) error {
	...
	if err != nil {
		log.Error().Err(err).Int("meeting_id", meetingID).Int("user_id", userID).Msg("Failed to add participant to meeting")
		return err
	}
	return nil
}
...
func (mr *meetingRepository) RemoveParticipantFromMeeting(ctx context.Context, meetingID, userID int) error {
	_, err := mr.db.NewDelete().
		...
		Exec(ctx)
	if err != nil {
		log.Error().Err(err).Int("meeting_id", meetingID).Int("user_id", userID).Msg("Failed to remove participant from meeting")
		return err
	}
	return nil
}
